...
module part(){
    ...
    extended_catchment_inverted_pyramid();
}
}
...
module extended_catchment_inverted_pyramid(){
    xx = 1;
    yy= 1;
    zz = 1;
    translate([catchment_x/4 +2.2, catchment_y/2 + 1, catchment_z+epsilon])

    rotate([180,0,0])
    scale([0.99*catchment_x/2, 0.95*catchment_y/2,catchment_z])
    polyhedron(
        points=[
            [xx,yy,0],[xx,-yy,0],[-xx,-yy,0],[-xx,yy,0], // the four points at base
            [0,yy/4,zz]
            ],                               // the apex point 
        faces=[
